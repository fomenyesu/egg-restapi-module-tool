...
let authHeader ={};
    ...
    if (url.includes("/user/authorize")) {
      pdata ={grant_type:"password",username:data.username,password:data.password};
      ...
      if (tokenData.constructor.name!="Error") {
        ...
        Cookie.set("SESSION_TOKEN","Bearer "+tokenData.access_token,{ expires: date })
        authHeader = {headers:{"Authorization" : "Bearer "+tokenData.access_token}};
      }
...
function refleshToken (pdata) {
  return new Promise((resolve, reject) => {
    //如果没有或者过期，重新获取token
    axios.post(config.devBaseURL +'/oauth2/access_token', {grant_type:"password",...pdata}, {
      method: 'post',
      ...
      headers: { 
        ...
        'Authorization': 'Basic ZWdnQ2xpZW50OlNEMTIzZGZqaGdpeTI4ZHNqa2ZiaTEyaHUzdWk='
      }
    })
