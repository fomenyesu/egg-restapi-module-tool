...
describe("test/service/restql.test.js", () => {
  ...
  describe("index()", () => {
    it("should index exists restql", function*() {
      ...
      const result = yield ctx.service.restql.index(
        "web_admin",
        { page: 1, pageSize: 5 },
        { status: 1 }
      );
  ...
  describe("update()", () => {
    it("should update exists restql", function*() {
      const ctx = app.mockContext();
      const result = yield ctx.service.restql.update("web_admin", 1, {
        status: 0
      });
      assert(result);
      // assert(result.uid==5);
    ...
    it("should get null when restql not exists", function*() {
      const ctx = app.mockContext();
      const result = yield ctx.service.restql.update("web_admin", 10000, {
        status: 0
      });
  ...
  describe("create()", () => {
    it("should create exists restql", function*() {
      const ctx = app.mockContext();
      const restql = yield ctx.service.restql.create("web_admin", {
        name: "aboutus",
        pass: "httml",
        status: 1,
        time: "1325472736"
